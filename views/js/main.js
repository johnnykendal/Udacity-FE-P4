...
// resizePizzas(size) is called when the slider in the "Our Pizzas" section of the website moves.
  ...
  // Changed querySelector to getElementById because it is a faster method 
  ...
  function changePizzaSizes(size) {
    // Changed querySelector to getElementById because it is a faster method and created a varible to hold it to cut down loading time
	var pizzaContainerValue = document.getElementsByClassName("randomPizzaContainer");
    // Moved layout code outside of the for-loop to improve rendering time
    var dx = determineDx(pizzaContainerValue[0], size);
    var newWidth = (pizzaContainerValue[0].offsetWidth + dx) + 'px';
	// Created len varible to store the length so that it is not being called every time inside the for-loop
	var len = pizzaContainerValue.length; 
	for (var i = 0; i < len; i++) {
...
function updatePositions() {
  ...
  //getElementsByClassName is a faster than query
  var items = document.getElementsByClassName('mover');
  var top = document.body.scrollTop / 1250;
  
  for (var i = 0; i < 5; i++) {
    phase.push(Math.sin((top) + i));
	console.log("----");
    console.log("i = " + i);
  ...
  for (var i = 0; i < items.length; i++){
    var pixel = items[i].style.left = items[i].basicLeft + 100 * phase[i%5] + 'px';
...
window.addEventListener('scroll', requestAnimationFrameScolling);

function requestAnimationFrameScolling() {
	if (!animating) {
...
window.addEventListener("DOMContentLoaded", function() {
 var cols = 8;
 var s = 256;
 // Dynamic changing varibles depending on sceen size to calculate the amount of pizzas needed to be created and have them outside the for loop so that it is only called once when the website is called
 var colNumber = Math.ceil(window.innerWidth / s);
 var rowNumber = Math.ceil(window.innerHeight / s);
 var PizzasOnScreen = colNumber * rowNumber; 
 var appendElem = document.getElementById("movingPizzas1");

 // Changed i to to varible PizzasOnScreen to have dynamic changing amount of pizzas on the screen dpending on the screen size to optimize performance
 // Created the varible elem outside of the for loop to prevent having it created everytime the loop runs and then declaring what it is equal to inside of the loop
 var elem;
